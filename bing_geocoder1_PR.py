...
encodage = 'ANSI'
entry_file = "inputs/PR_reste.csv"
...
with open(entry_file, newline='', encoding=encodage) as csvfile:
    ...
    for row in reader:
        if(int(row['ogc_fid']) > 41235):

            res = json.loads(urllib.request.urlopen(getUrl(row['ville_site'], row['adresse_brute'], key)).read())  # .decode('utf-8')
            time.sleep(0.15)
